...
日期: 2026-07-08 | 状态: [DONE]

## 概述

将 Bot 人设从「小夏·18岁·温暖文艺少女」升级为「夏柠·20岁·腹黑傲娇天然憨·二次元抽象人」，同时利用现有多脑架构（辅脑/主脑/评估脑）实现分层人设注入——不同脑使用不同深度的 persona。
...
### full_persona（主脑深度回复 + 看图，~900 tokens）
...
graph TB
    subgraph "memory_store"
        CORE["global/persona/core\n(core_persona ~400t)"]
        FULL["global/persona/full\n(full_persona ~900t)"]
        EVAL["global/persona/eval\n(eval_guidelines ~150t)"]
    end

    subgraph "engine._assemble_system_prompt()"
        ASM["根据 role 参数选择层级"]
    end

    FB["辅脑 chat(role='fast')"] --> ASM
    SB["主脑 chat(role='strong')"] --> ASM
    EB["评估脑 evaluate()"] --> ASM
...
### 1. `orchestrator.init_memory()` — 初始化 3 层 persona

替换当前单个 `global/persona/core` 写入为 3 个条目写入。

### 2. `engine._assemble_system_prompt()` — 分层注入

当前只从 `global/persona/core` 拉取。需改为：

```python
async def _assemble_system_prompt(user_id, is_new_session, user_message="", persona_level="full"):
    if persona_level == "core":
        key = "global/persona/core"
    elif persona_level == "eval":
        key = "global/persona/eval"
    else:
        key = "global/persona/full"
    # ... fetch persona_entry from key
```

### 3. `engine.chat()` — 透传 persona_level

`chat()` 已有 `role` 参数。新增 `persona_level` 参数，默认 `"full"`，辅脑调用时传 `"core"`。

### 4. `reply_handler.handle_reply()` — 辅脑用 core

图片消息的辅脑和纯文本的辅脑都使用 `core_persona`。

### 5. `reply_scheduler._evaluate_and_followup()` / `reply_handler.evaluate_and_followup()` — 评估用 eval

评估时注入 `eval_guidelines` 而非完整人设。

## 实现清单

| # | 文件 | 改动 |
|---|------|------|
| 1 | `orchestrator.py` | `init_memory()` 写入 core/full/eval 三条 persona |
| 2 | `engine.py` | `_assemble_system_prompt()` 接受 `persona_level` 参数 |
| 3 | `engine.py` | `chat()` 接受并透传 `persona_level` |
| 4 | `reply_handler.py` | 辅脑调用 `chat()` 时传 `persona_level="core"` |
| 5 | `reply_handler.py` | `evaluate_and_followup()` 注入 eval_guidelines |
| 6 | `reply_handler.py` | 主脑调用 `chat()` 时传 `persona_level="full"` |

## 迁移策略

1. 更新 `init_memory()` 写入 3 条新 persona
2. 已存在的 `global/persona/core` 会被新内容覆盖（首次启动时自动执行）
3. 如需保留旧人设，可先备份 `memory_store` 中的旧条目
4. 零停机：重启后新 persona 生效
