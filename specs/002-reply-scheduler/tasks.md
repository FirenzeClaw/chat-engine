...
- [ ] T022 [US3] Implement `_analyze_frequency(actor)` in `reply_scheduler.py`: sliding window over `speakers_history` deque, count unique speakers, return ACTIVE/QUIET/IDLE
- [ ] T023 [US3] Implement `_background_tick()` in `reply_scheduler.py`: every 1s iterate all WAITING group actors, update speakers_history (prune old entries), call `_analyze_frequency()`
...
- [ ] T025 [US3] Wire chime-in trigger in `_background_tick()`: if `monotonic >= chime_at` and still ACTIVE → trigger actor with P3 priority
- [ ] T026 [US3] Wire IDLE trigger in `_background_tick()`: if IDLE with buffered group messages → trigger actor with P4 priority
